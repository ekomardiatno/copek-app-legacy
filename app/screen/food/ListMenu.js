...
import { View, Text, Alert, ActivityIndicator, StatusBar, Platform, ScrollView } from 'react-native'
...
export default class ListMenu extends Component {
  ...
  _promiseFetch = () => {
    ...
    return new Promise((resolve, reject) => {
      fetch(`${HOST_REST_API}food/get?koordinat=${position.latitude},${position.longitude}&orderby=${orderBy}&kota=${cityName}&page=${page}`)
        .then(res => res.json())
        .then(resolve)
        .catch(reject)
    })
  ...
  render() {
    const { data, page, isFetchReached, isLoaderViewed, scrolling, status } = this.state
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={[{ backgroundColor: Color.white }]}>
          <SimpleHeader goBack navigation={this.props.navigation} title='Daftar Menu' />
        </View>
        {
